...
export async function fetchInventoryTransactionsByManagerID(start_date, end_date, from_location, product_name) {
  try {
    const queryParams = `?start_date=${encodeURIComponent(start_date)}&end_date=${encodeURIComponent(end_date)}&to_location=${encodeURIComponent(from_location)}&product_name=${encodeURIComponent(product_name)}`;
